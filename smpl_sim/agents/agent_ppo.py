    ...
    def update_policy(self, states, critic_states, actions, returns, advantages, exps):
        ...
        for _ in range(self.opt_num_epochs):
            if self.use_mini_batch:
                ...
                states, critic_states, actions, returns, advantages, fixed_log_probs, exps = \
                    states[perm].clone(), critic_states[perm].clone(), actions[perm].clone(), returns[perm].clone(), advantages[perm].clone(), \
                ...
                for i in range(optim_iter_num):
                    ...
                    states_b, critic_states_b, actions_b, advantages_b, returns_b, fixed_log_probs_b, exps_b = \
                        states[ind], critic_states[ind], actions[ind], advantages[ind], returns[ind], fixed_log_probs[ind], exps[ind]
                    ind = exps_b.nonzero(as_tuple=False).squeeze(1)
                    self.update_value(critic_states_b, returns_b)
